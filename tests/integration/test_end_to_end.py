...
class TestFullPipeline:
    ...
    def test_full_sequence(self, migrated_db,
                           soc_hierarchy_content, soc_definitions_content,
        ...
        tax_result = taxonomy_refresh(
            migrated_db, soc_hierarchy_content, soc_definitions_content, soc_ver, soc_ver,
        )
        assert tax_result.status == PipelineStatus.SUCCESS

        # oews_refresh
        oews_result = oews_refresh(
            migrated_db, oews_national_content, oews_state_content, oews_rel, soc_ver,
        )
        ...
        onet_result = onet_refresh(
            ...
            onet_abilities_content, onet_tasks_content,
            onet_ver, onet_ver, soc_ver,
        )
        assert onet_result.status == PipelineStatus.SUCCESS

        # projections_refresh
        proj_result = projections_refresh(
            migrated_db, projections_content, proj_rel, "2022-2032", soc_ver,
        )
        ...
        for run_id in [tax_result.run_id, oews_result.run_id,
                       onet_result.run_id, proj_result.run_id]:
            run = get_run(migrated_db, run_id)
            assert run["load_status"] == "success"

        # All mart views queryable
        for view in ["occupation_summary", "occupation_wages_by_geography",
...
class TestAnalystQueries:
    ...
    def test_wage_distribution_by_geography(self, onet_loaded_db):
        ...
        rows = onet_loaded_db.execute(
            "SELECT geo_name, mean_annual_wage"
...
class TestFullIdempotence:
    ...
    def test_full_rerun_no_duplicates(self, migrated_db,
                                       soc_hierarchy_content, soc_definitions_content,
                                       oews_national_content, oews_state_content,
                                       onet_skills_content, onet_knowledge_content,
                                       onet_abilities_content, onet_tasks_content):
        ...
        onet_refresh(migrated_db, onet_skills_content, onet_knowledge_content,
                     onet_abilities_content, onet_tasks_content, onet_ver, onet_ver, soc_ver)

        tables = ["dim_occupation", "fact_occupation_employment_wages",
                  "dim_skill", "bridge_occupation_skill", "bridge_occupation_task"]
